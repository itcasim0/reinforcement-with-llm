...
# 한글 기준으로 평가할 떄에는 아래 코드를 주석 풀고 위에를 주석 걸어서 사용할 것.        
import re
import numpy as np
...
class AbstractQualityEvaluator:
    ...
    def _init_english_keywords(self):
        ...
        self.vague_terms = ['various', 'several', 'many', 'some', 'few', 'often', 'sometimes']
    ...
    def _init_korean_keywords(self):
        """한국어 키워드 초기화"""
        self.structure_keywords = {
            'background': ['배경', '맥락', '동기', '문제', '과제', '기존', '현재', '기존의', '현재의'],
            'objective': ['목적', '목표', '목적은', '목표는', '연구', '조사', '탐구', '분석', '규명', '밝히'],
            'method': ['방법', '접근', '기법', '알고리즘', '프레임워크', '모델', '제안', '제시', '개발', '설계'],
            'result': ['결과', '발견', '입증', '보여', '달성', '성능', '실험', '분석', '확인'],
            'conclusion': ['결론', '시사점', '제안', '기여', '의의', '중요', '기대', '향후']
        }
        
        self.academic_connectives = [
            '그러나', '하지만', '또한', '더욱이', '따라서', '그러므로', 
            '그럼에도', '결과적으로', '특히', '구체적으로', '즉',
            '반면', '한편', '나아가', '이에', '이를 통해'
        ]
        ...
        # 한국어 모호한 표현
        self.vague_terms = ['여러', '다양한', '몇몇', '일부', '종종', '때때로', '가끔', '많은']
    ...
    def evaluate_length(self, abstract: str) -> Dict[str, float]:
        ...
        if self.optimal_word_count_min <= word_count <= self.optimal_word_count_max:
            ...
            length_score = max(0, 1.0 - (word_count - self.optimal_word_count_max) / self.optimal_word_count_max)
        
        # 문장 길이 점수
        avg_words_per_sentence = word_count / max(1, sentence_count)
        
        if self.optimal_words_per_sentence_min <= avg_words_per_sentence <= self.optimal_words_per_sentence_max:
            sentence_length_score = 1.0
        else:
            sentence_length_score = max(0, 1.0 - abs(avg_words_per_sentence - self.target_words_per_sentence) / self.target_words_per_sentence)
...
if __name__ == "__main__":
    ...
    print(f"  Length: {results_en['length']['overall_length_score']:.3f} ({results_en['length']['word_count']} words)")
    print(f"  Academic Style: {results_en['academic_style']['academic_style_score']:.3f}")
    print(f"  Information: {results_en['information_density']['information_density_score']:.3f}")
    ...
    print(f"  길이: {results_ko['length']['overall_length_score']:.3f} ({results_ko['length']['word_count']} 어절)")
    print(f"  학술 스타일: {results_ko['academic_style']['academic_style_score']:.3f}")
    print(f"  정보 밀도: {results_ko['information_density']['information_density_score']:.3f}")
