...
class A2CRunner:
    """
    A2C (Advantage Actor-Critic) 학습 러너
    
    - 환경에서 rollout 수집
    - A2C loss 계산 및 업데이트
    
    A2C는 PPO와 달리 clipping 없이 바로 policy gradient를 사용하며,
    수집한 데이터로 한 번만 업데이트합니다.

    Args:
        env (EditingEnv): 강화학습 환경 (문서 편집 환경)
        max_steps (int): 에피소드당 최대 스텝 수
        num_actions (int): 가능한 액션의 개수 (편집 작업 종류)
        gamma (float): 할인율 (discount factor) - 미래 보상의 가중치. 기본값 0.99
        gae_lambda (float): GAE lambda 파라미터 - advantage 추정의 bias-variance 조절. 기본값 0.95
        entropy_coef (float): Entropy 계수 - 탐색 장려. 기본값 0.01
    ...
    def __init__(
        ...
        # A2CPolicy(신경망 모델) 하이퍼파라미터
        self.gamma = gamma
        self.gae_lambda = gae_lambda
        self.entropy_coef = entropy_coef

        self.policy = A2CPolicy(state_dim=state_dim, num_actions=num_actions).to(
            self.device
        )
    ...
    def a2c_update(self, traj):
        """
        A2C 업데이트 수행
        
        A2C는 수집한 trajectory로 한 번만 업데이트합니다.
    ...
    def train(
        ...
        for ep in range(self.start_episode + 1, num_episodes + 1):
            ...
            # A2C 업데이트 (매 에피소드마다 업데이트)
            loss_info = self.a2c_update(traj)
