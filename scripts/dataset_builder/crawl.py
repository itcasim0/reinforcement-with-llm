...
class LargeScaleAnalyzer:
    ...
    def collect_papers_large_scale(self, target_count=500):
        ...
        print(
            f"   연도 범위: {min(p['year'] for p in papers)} ~ {max(p['year'] for p in papers)}"
        )
    ...
    def analyze_abstracts_comprehensive(self):
        ...
        analysis = {
            "metadata": {
                ...
                "year_range": [
                    min(p["year"] for p in self.papers),
                    max(p["year"] for p in self.papers),
                ],
            },
    ...
    def _analyze_linguistic_features(self, abstracts):
        ...
        for abstract in abstracts:
            ...
            if any(
                w in abs_lower
                for w in ["better than", "compared to", "outperform", "superior", "vs"]
            ):
    ...
    def _print_comprehensive_analysis(self, analysis):
        ...
        print(f"\n단어 수 통계:")
        print(
            f"  평균: {stats['word_count']['mean']:.1f} ± {stats['word_count']['std']:.1f}"
        )
        print(f"  중앙값: {stats['word_count']['median']:.1f}")
        print(f"  범위: {stats['word_count']['min']} ~ {stats['word_count']['max']}")
        print(
            f"  Q10-Q90: {stats['word_count']['q10']:.0f} ~ {stats['word_count']['q90']:.0f}"
        )
        print(
            f"  Q25-Q75: {stats['word_count']['q25']:.0f} ~ {stats['word_count']['q75']:.0f}"
        )
        print(
        ...
        print(
            f"  매우 짧음 (<100):     {dist['very_short']*100:5.1f}%  {'*' * int(dist['very_short']*50)}"
        )
        print(
            f"  짧음 (100-150):       {dist['short']*100:5.1f}%  {'*' * int(dist['short']*50)}"
        )
        print(
            f"  중간 (150-200):       {dist['medium']*100:5.1f}%  {'*' * int(dist['medium']*50)}"
        )
        print(
            f"  김 (200-250):         {dist['long']*100:5.1f}%  {'*' * int(dist['long']*50)}"
        )
        print(
            f"  매우 김 (250+):       {dist['very_long']*100:5.1f}%  {'*' * int(dist['very_long']*50)}"
        )
        ...
        for cat, stats in sorted(analysis["category_comparison"].items()):
            print(
                f"{cat:10s} | {stats['count']:3d}개 | 평균 {stats['avg_length']:.0f}단어"
            )
    ...
    def create_evaluation_model(self, analysis):
        ...
        print(f"\n1. 길이 점수 (15%):")
        print(
            f"   최적: {model['scoring_criteria']['length_score']['optimal_min']:.0f}-{model['scoring_criteria']['length_score']['optimal_max']:.0f} 단어"
        )
        print(
            f"   목표: {model['scoring_criteria']['length_score']['target']:.0f} 단어"
        )
